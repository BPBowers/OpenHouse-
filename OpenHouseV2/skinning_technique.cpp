/**
\file skinning_technique.cpp
\brief Program file for skinning_technique.h, a lot of functions are ignored by my engine
...
*/
//*
    if (!AddShader(GL_VERTEX_SHADER, "ObjVertexShader.glsl")) {
        return false;
    }

    if (!AddShader(GL_FRAGMENT_SHADER, "ObjFragmentShader.glsl")) {
        return false;
    }
//*/
    if (!Finalize()) {
    ...
    WVPLoc = GetUniformLocation("PVM");
    //materialLoc = GetUniformLocation("Mat");
...
void SkinningTechnique::SetWVP(glm::mat4& WVP){
    //Matrix4f ogWVP;
    //ogWVP.InitIdentity();
    //glUniformMatrix4fv(WVPLoc, 1, GL_TRUE, (const GLfloat*)ogWVP.m);
    //glUniformMatrix4fv(WVPLoc, 1, GL_TRUE, (const GLfloat*)WVP.m);//This is the original
...
void SkinningTechnique::SetMaterial(const oMaterial& material)
{
    ...
    /*
    glUniform4fv(glGetUniformLocation("Mat.ambient"), 1, glm::value_ptr(mat.getAmbient()));
    glUniform4fv(glGetUniformLocation("Mat.diffuse"), 1, glm::value_ptr(mat.getDiffuse()));
    glUniform4fv(glGetUniformLocation("Mat.specular"), 1, glm::value_ptr(mat.getSpecular()));
    */
}
